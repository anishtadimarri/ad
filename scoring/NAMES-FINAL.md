# Every Name Considered, Ranked Once

> The naming question has been asked five times. This is the consolidated answer, so it
> does not need asking again. **Availability was never the binding constraint — collision
> was.**

Checked across the whole engagement: **~4,400 RDAP checks** in [`domains.py`](domains.py), [`arbitrary.py`](arbitrary.py),
[`plain_sweep.py`](plain_sweep.py), [`idioms.py`](idioms.py) and [`co_names.py`](co_names.py), plus **2,662 more**
in the SkillForce-shape sweep. Verified August 2026.

---

## The ranking

| # | Domain | Status | `OWNED` | `CLEAN` | `REGISTER` | `NEUTRAL` | `EXPAND` | `SAY` | `TLD` | **Score** |
|---|---|---|---|---|---|---|---|---|---|---|
...
| Dimension | Weight | |
|---|---|---|
| `OWNED` | 20 | Do we already own it? Switching costs real weeks, not dollars |
...
**1. `allhandstalent.com`** — 87

Idiom-led, which puts it nearer the Somewhere/Oceans register than the staffing compounds. *Talent* is the category's most generic noun and *all hands* collides with the meeting — but no company owns it, and it is method-neutral
...
> two weeks of sourcing. Every claim on the landing page depends on those existing, and
> none of it can be tested until they do.
